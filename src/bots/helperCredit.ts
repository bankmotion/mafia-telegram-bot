import { Context, Telegraf } from "telegraf";
import { Update } from "telegraf/typings/core/types/typegram";
import { ChainType } from "../enums/ChainType";
import { Config, Contract } from "../config/config";
import { getContract } from "../utils/contract";
import Web3 from "web3";

import HelperCreditABI from "../abis/HelperCredit.json";
import { EventName } from "../enums/EventName";
import {
  getBlockNumberFromName,
  updateBlockNumber,
} from "../services/blockInfoService";
import { BlockName } from "../enums/BlockInfo";
import { ethers } from "ethers";

function escapeMarkdown(text: string) {
  return text.replace(/[_*[\]()~`>#+\-=|{}.!\\]/g, "\\$&");
}

const sendMessage = async (
  bot: Telegraf<Context<Update>>,
  chain: ChainType,
  amount: number,
  address: string
) => {
  for (const chatId of Config.BotCratePurchaseChatID) {
    console.log(
      `\`${address}\` purchased ${escapeMarkdown(
        Math.floor(amount).toString()
      )} Credits \\(${escapeMarkdown(
        Math.floor(amount / 20).toString()
      )}\\\$\\) on ${chain}`
    );
    await bot.telegram.sendMessage(
      chatId,
      `\`${address}\` purchased ${escapeMarkdown(
        Math.floor(amount).toString()
      )} Credits \\(${escapeMarkdown(
        Math.floor(amount / 20).toString()
      )}\\\$\\) on ${chain}`,
      {
        parse_mode: "MarkdownV2",
      }
    );
  }
};

const getPastEvents = async (
  from: number,
  to: number,
  chain: ChainType,
  bot: Telegraf<Context<Update>>
) => {
  const toBlock = Math.min(from + 9000, to);
  const helperCreditContract = getContract(
    chain,
    HelperCreditABI,
    Contract.HelerCredit[chain]
  );

  const pastEvents = await helperCreditContract.getPastEvents(
    EventName.CreditBought,
    {
      fromBlock: from,
      toBlock,
    }
  );

  await updateBlockNumber(BlockName.CreditBought, toBlock, chain);

  for (let index = 0; index < pastEvents.length; index++) {
    const event: any = pastEvents[index];
    const buyer = event.returnValues.buyer;
    const creditAmount = ethers.formatEther(event.returnValues.creditAmount);
    await sendMessage(bot, chain, Number(creditAmount), buyer);
  }

  if (toBlock < to) {
    await getPastEvents(from + 9001, to, chain, bot);
  }
  console.log("scan finished for helper credit purcahse");
};

const start = async (chain: ChainType, bot: Telegraf<Context<Update>>) => {
  try {
    const web3 = new Web3(Config.RPCProvider[chain]);

    const fromBlock = await getBlockNumberFromName(
      BlockName.CreditBought,
      chain
    );

    if (!fromBlock) return;

    const toBlock = await web3.eth.getBlockNumber();

    console.log({ fromBlock, toBlock });
    await getPastEvents(fromBlock + 1, Number(toBlock), chain, bot);
  } catch (err) {
    console.error(`Error in cratePurchase ${err}`);
  }
};

export const scanCreditBought = async (
  chain: ChainType,
  bot: Telegraf<Context<Update>>
) => {
  setInterval(() => {
    start(chain, bot);
  }, 600 * 1000);

  start(chain, bot);
};
